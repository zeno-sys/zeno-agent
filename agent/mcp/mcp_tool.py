    ...
    def _convert_messages(self, params) -> List[dict]:
        ...
        for msg in params.messages:
            ...
            if tool_uses:
                ...
                from agent.core.multimodal import mcp_content_blocks_to_openai_parts

                parts = mcp_content_blocks_to_openai_parts(content_blocks)
                if not parts:
                    ...
                    messages.append({"role": msg.role, "content": parts[0]["text"]})
                else:
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        server_params = StdioServerParameters(
            ...
            encoding="utf-8",
            # Windows 下个别 MCP 进程可能输出非 UTF-8 字节，避免直接解码崩溃
    ...
    async def _run_http(self, config: dict):
        ...
        if self._auth_type == "oauth":
            try:
                from agent.mcp.mcp_oauth_manager import get_manager
...
def _get_auth_error_types() -> tuple:
    ...
    try:
        ...
        pass
    try:
        from agent.mcp.mcp_oauth import OAuthNonInteractiveError
...
def _ensure_mcp_loop():
    ...
    loop_started_here: Optional[asyncio.AbstractEventLoop] = None
    with _lock:
        ...
        _mcp_thread.start()
        loop_started_here = _mcp_loop

    # run_forever() runs on another thread; without waiting, the next
    # _run_on_mcp_loop() can observe is_running() == False and raise.
    if loop_started_here is not None:
        deadline = time.monotonic() + 10.0
        while time.monotonic() < deadline:
            if loop_started_here.is_running():
                return
            time.sleep(0.001)
        raise RuntimeError(
            "MCP event loop failed to start (background thread did not enter run_forever)"
        )
...
def _run_on_mcp_loop(coro, timeout: float = 30):
    ...
    Poll in short intervals so timeout checks stay responsive while
    MCP work is running on the background loop.
    """
    # TODO: 如有需要，在这里恢复“用户中断”轮询逻辑。
    # 之前这里会在每次轮询时检查 tools.interrupt.is_interrupted()，
    # 若检测到用户有新消息，则取消 MCP future 并抛出 InterruptedError，
    # 让新消息可以抢占正在执行的长耗时 MCP 调用。
    # 目前该逻辑已暂时关闭，MCP 调用会一直执行到完成或超时。
    with _lock:
        loop = _mcp_loop
    if loop is None or not loop.is_running():
...
def _load_mcp_config() -> Dict[str, dict]:
    try:
        ...
        import yaml
        config = yaml.load(Path("data/config/mcp.yaml").read_text(), Loader=yaml.FullLoader)
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    ``handler(args, extra_args) -> str``
    """

    def _handler(args: dict, extra_args: dict | None = None) -> str:
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    for mcp_tool in server._tools:
        ...
        registry.register(
            ...
            schema=schema,
            handler=lambda args, extra_args, sn=name, tn=mcp_tool.name, tt=server.tool_timeout: _make_tool_handler(
                sn, tn, tt
            )(args, extra_args),
    ...
    for entry in _select_utility_schemas(name, server, config):
        ...
        registry.register(
            ...
            toolset=toolset_name,
            schema=schema,
            handler=lambda args, extra_args, _h=handler: _h(args, extra_args),
            check_fn=check_fn,
            emoji="🔌",
        )
...
def get_mcp_status() -> List[dict]:
    ...
    with _lock:
        # 把当前 _servers（已经连接的MCP服务器） 浅拷贝一份出来,避免在遍历过程中修改 _servers
