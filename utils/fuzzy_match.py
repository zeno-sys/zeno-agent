...
实现了一套多策略匹配链，用于鲁棒地查找和替换文本，
以适应LLM生成代码中常见的空白、缩进、转义等差异。

8种匹配策略链（受OpenCode启发），依次尝试如下：
1. 精确匹配 - 直接字符串对比
2. 行去空格匹配 - 每行去除首尾空白再比对
3. 空白规范化 - 合并多个空格/制表位为一个空格
4. 缩进灵活 - 完全忽略缩进差异
5. 转义处理 - 将 \\n 字符串变为实际换行
6. 边界去空格 - 仅去除首行/末行的空白再比对
7. 块锚定匹配 - 首行+末行强匹配，中间用相似度辅助
8. 上下文模糊感知 - 至少50%的行相似（阈值为50%）

多次出现的匹配通过 replace_all 参数处理。
...
为什么需要它？
    模型生成的 old_string 常见偏差包括：多空格/制表符、缩进不一致、\n 字面量与
    真实换行、智能引号/破折号等 Unicode、以及工具链里偶发的转义漂移。模块用一串
    从严到松的策略依次尝试匹配，尽量在「仍能安全写回文件」的前提下完成替换。
"""
