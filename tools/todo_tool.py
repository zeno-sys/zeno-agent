...
本模块为代理提供一个内存中的任务清单，用于分解复杂任务、跟踪进度，并在长时间对话中保持关注重点。任务状态存储在每个
AIAgent 实例上（每个会话一个），并在上下文压缩事件后重新注入到对话中。

设计：
- 单一 `todo` 工具：传入 `todos` 参数可写入，不传则读取
- 每次调用都会返回当前全部任务列表
- 不会修改系统提示词，也不会修改工具响应
- 用户行为引导全部包含在工具 schema 的描述中
"""
