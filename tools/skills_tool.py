...
from constants import display_zeno_home, get_zeno_home, get_skills_dir
...
def skill_matches_platform(frontmatter: Dict[str, Any]) -> bool:
    ...
    """
    from agent.skill.skill_utils import skill_matches_platform as _impl
...
def _parse_frontmatter(content: str) -> Tuple[Dict[str, Any], str]:
    ...
    Delegates to ``agent.skill.skill_utils.parse_frontmatter`` — kept here
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    try:
        from agent.skill.skill_utils import get_external_skills_dirs
...
def _get_disabled_skill_names() -> Set[str]:
    ...
    Delegates to ``agent.skill.skill_utils.get_disabled_skill_names`` — kept here
    as a public re-export so existing callers don't need updating.
    """
    from agent.skill.skill_utils import get_disabled_skill_names
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """递归查找所有技能 in 默认目录和外部目录。"""
    from agent.skill.skill_utils import get_external_skills_dirs, iter_skill_index_files
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_zeno_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def skill_view(
    ...
    try:
        ...
        if ":" in name:
            from agent.skill.skill_utils import is_valid_namespace, parse_qualified_name
        ...
        if not skill_md:
            for search_dir in all_dirs:
                from agent.skill.skill_utils import iter_skill_index_files
        ...
        if file_path and skill_dir:
            from agent.tool.path_security import validate_within_dir, has_traversal_component
