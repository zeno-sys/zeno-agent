...
from constants import get_skills_dir, display_zeno_home
...
def atomic_replace(tmp_path: Union[str, Path], target: Union[str, Path]) -> str:
    """以原子方式将*tmp_path*移动到*target*，并保留符号链接。"""
...
def _containing_skills_root(skill_path: Path) -> Path:
    ...
    ``_find_skill`` first).
    """
    from agent.skill.skill_utils import get_all_skills_dirs
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    {"path": Path} or None.
    """
    from agent.skill.skill_utils import EXCLUDED_SKILL_DIRS, get_all_skills_dirs
...
def _validate_file_path(file_path: str) -> Optional[str]:
    ...
    """
    from agent.tool.path_security import has_traversal_component
...
def _resolve_skill_target(skill_dir: Path, file_path: str) -> Tuple[Optional[Path], Optional[str]]:
    ...
    from agent.tool.path_security import validate_within_dir
...
def _patch_skill(
    ...
    if match_error:
        ...
        err_msg = match_error
        try:
            from utils.fuzzy_match import format_no_match_hint
...
def skill_manage(
    ...
    if result.get("success"):
        # TODO: 技能文件被改过后，应该在下一轮对话里的使用新的技能说明，以确保实时生效
        pass
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {get_skills_dir()}; existing skills can be modified wherever they live.\n\n"
